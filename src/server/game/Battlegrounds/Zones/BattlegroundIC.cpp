...
Transport* BattlegroundIC::MakeTransport(uint32 gobentry,uint32 period,std::string nametransport)
{
    Transport *t = new Transport(period,0);
        ...
        QueryResult npc_transport = WorldDatabase.PQuery("SELECT npc_entry, TransOffsetX, TransOffsetY, TransOffsetZ, TransOffsetO, emote, guid FROM creature_transport WHERE npc_entry = '%u'", entry);
        if(npc_transport)
        {
            do
            {

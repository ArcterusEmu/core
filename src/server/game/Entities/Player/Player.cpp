...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
    ...
    if (sWorld.getBoolConfig(CONFIG_GMISLAND_PLAYERS_ACCESS) == true)
    {
        if (newZone == 876 && GetSession()->GetSecurity() == SEC_PLAYER)
        {
            ...
            if (result)
            {
                ...
                if (map != 876 && sWorld.getBoolConfig(CONFIG_GMISLAND_BAN_ENABLE) == false)
                {
                    sLog.outError("Player (GUID: %u) tried to access GM Island. Sending possible hacker to %u", GetGUIDLow(), map);
                ...
                if (sWorld.getBoolConfig(CONFIG_GMISLAND_BAN_ENABLE) == true)
                {
                    ...
                    sWorld.BanAccount(BAN_ACCOUNT, this->GetName(),secsToTimeString(TimeStringToSecs("-1"),true).c_str(),"Du warst unerlaubt auf der GMInsel","Monkeys AntiCheat Protection");
                }
            }
        }
    }
    ...
    // Prevent players from accessing unused areas
    if (sWorld.getBoolConfig(CONFIG_PLAYERS_ACCESS_UNUSED_AREA) == true)
    {
        if (newZone == 876 && GetSession()->GetSecurity() == SEC_PLAYER)
        {
            QueryResult result = CharacterDatabase.PQuery("SELECT * FROM `unused_area_teleport` WHERE 1");
            ...
            if (result)
            {
                ...
                if (map == 876)
                {
                    sLog.outError("Player (GUID: %u) tried to access unused area.", GetGUIDLow());
                    sLog.outError("Error: Cannot set tele to unused area (map: %u). Sending possible hacker to default location. (Jail Box)",map);
                    TeleportTo(13,1.118799,0.477914,-144.708650,3.133046); // Tele to Jail Box
                }

                if (map != 876 && sWorld.getBoolConfig(CONFIG_UNUSED_AREA_BAN_ENABLE) == false)
                {
                    sLog.outError("Player (GUID: %u) tried to access unused area. Sending possible hacker to %u", GetGUIDLow(), map);
                    TeleportTo(map,x,y,z,orientation);
                }

                if (sWorld.getBoolConfig(CONFIG_UNUSED_AREA_BAN_ENABLE) == true)
                {
                    sLog.outError("Player (GUID: %u) tried to access unused area. Banning Player Account.", GetGUIDLow());
                    sWorld.BanAccount(BAN_ACCOUNT, this->GetName(),secsToTimeString(TimeStringToSecs("-1"),true).c_str(),"Unerlaubter Aufenthalt in einer Unbenutzen Area","Monkeys AntiCheat Protection");
                }
            }
        }
    }
